...
;===================================================================================================
; Function........:  ListProcesses($h_L_Processes, $ProcessList, $Mode, $CurrentSessionOnly, $Filter)
;
...
;===================================================================================================
; Function........:  CreateProcessList($ProcessList)
;
; Description.....:  Creates a GUI in which all currently running processes are listed.
;
; Parameter(s)....:  $ProcessList - An array returned by ProcessList().
;
...
Func ListProcesses($h_L_Processes, $ProcessList, $Mode, $CurrentSessionOnly, $Filter)
...
Func CreateProcessList($ProcessList)
	...
	While (True)
		...
		Select
			...
			Case $l_L_DoubleClickedIndex <> -1
				...
				If ($lProcArch = "---") Then
					$mbRet = MsgBox(BitOR($MB_ICONWARNING, $MB_TOPMOST , $MB_YESNO), "Warning", "Cannot attach to the selected process. Do you want to select this process anyway?")
					If ($mbRet = $IDNO) Then
						ContinueLoop
					EndIf
				EndIf
			...
			Case $Msg = $h_L_List
				...
				If ($ClickedCol <> 0) Then
					...
					$ProcessCount = ListProcesses($h_L_List, $ProcessList, $ClickedCol - 1, $bSession, $g_ProcNameFilter)
					WinSetTitle($h_G_Select, "", "Select a process (" & $ProcessCount & ")")
				EndIf
			...
			Case $Msg = $h_C_CurrSession
				...
				$ProcessCount = ListProcesses($h_L_List, $ProcessList, 1, $bSession, $g_ProcNameFilter)
				WinSetTitle($h_G_Select, "", "Select a process (" & $ProcessCount & ")")
	...
	$g_CurrentSession = (BitAND(GUICtrlRead($h_C_CurrSession), $GUI_CHECKED) <> 0)
